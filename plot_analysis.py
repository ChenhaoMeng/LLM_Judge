...
def visualize_results(df_phenom, df_matrix, df_uid, stats):
    """
    可视化分析结果
    """
    # 设置中文字体
    ...
    plt.tight_layout()
    plt.savefig(f"{config['output']['visualization_dir']}/model_comparison_analysis.png", 
               dpi=300, bbox_inches='tight')
    plt.close()  # 关闭图形以释放内存
    print(f"📊 可视化图表已保存: {config['output']['visualization_dir']}/model_comparison_analysis.png")
...
if __name__ == "__main__":
    ...
    if df_05 is None or df_15 is None:
        ...
        # 4. 可视化
        visualize_results(df_phenom_stats, df_consistency, df_uid_stats, detailed_stats)
        ...
        print(f"  3. {config['output']['visualization_dir']}/model_comparison_analysis.png (可视化图表)")
        print(f"  4. {config['output']['visualization_dir']}/analysis_report.md (详细分析报告)")