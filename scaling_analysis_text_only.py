...
# -*- coding: utf-8 -*-
"""
Text-only Scaling Law Analysis for Language Models
This script analyzes scaling behavior of Qwen models focusing on specific phenomena without visualization.
"""
...
class ScalingTextOnlyAnalyzer:
    """
    Text-only analyzer for scaling law verification - focusing on specific linguistic phenomena
    """
    ...
    def __init__(self, config_path: str = None):
        ...
        if config_path and os.path.exists(config_path):
            ...
            self.config = {
                ...
                "output": {
                    "comparison_csv": "scaling_text_only_analysis_result.csv",
                    "uid_breakdown_csv": "scaling_text_only_uid_breakdown.csv",
    ...
    def generate_detailed_report(self, scaling_results: Dict, uid_stats: pd.DataFrame, model_names: List[str]):
        ...
        Args:
            ...
            model_names: List of model names
        """
        report_path = f"{self.config['output']['visualization_dir']}/scaling_text_only_analysis_report.md"

        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("# Scaling Law Analysis Report - Text Only\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='Scaling Law Analysis Tool - Text Only')
    parser.add_argument('--config', type=str, default='config.json', help='Configuration file path')
    args = parser.parse_args()

    analyzer = ScalingTextOnlyAnalyzer(config_path=args.config)
